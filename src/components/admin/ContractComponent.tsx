...
interface ContractComponentProps {
  data?: ContractData
  mode?: 'preview' | 'html' | 'pdf'
}

const defaultData: ContractData = {
  contractNumber: '………',
  contractDate: '......../……../…………',
  clientName: '(numele și prenumele)',
  clientAddress: {
    locality: '……………………………………………………………',
    street: '……………………………………………………………',
    number: '',
...
export function ContractComponent({ data = defaultData, mode = 'preview' }: ContractComponentProps) {
  const contractData = { ...defaultData, ...data }
  
  const containerClass = mode === 'pdf' 
    ? 'bg-white text-black font-serif max-w-none p-8'
    : 'bg-white text-black border rounded-lg p-8 max-w-4xl mx-auto font-serif text-sm leading-relaxed'
  ...
  return (
    <div className={containerClass}>
      <div className="text-center mb-8">
        ...
        <p className="mb-4">
          Nr {contractData.contractNumber} din {contractData.contractDate}
        </p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <section>
          ...
          <div className="mb-4">
            ...
            <p className="mb-2">
              Dl / Dna <span className="font-semibold">{contractData.clientName}</span> cu domiciliul
            </p>
            <p className="mb-2">
              în Localitatea <span className="font-semibold">{contractData.clientAddress?.locality}</span>, 
              Str. <span className="font-semibold">{contractData.clientAddress?.street}</span>, 
              Nr. <span className="font-semibold">{contractData.clientAddress?.number}</span>,
            </p>
            <p className="mb-2">
              Bl. <span className="font-semibold">{contractData.clientAddress?.building}</span>, 
              Sc. <span className="font-semibold">{contractData.clientAddress?.staircase}</span>, 
              Et. <span className="font-semibold">{contractData.clientAddress?.floor}</span>, 
              Ap. <span className="font-semibold">{contractData.clientAddress?.apartment}</span>, 
              Județul/Sectorul <span className="font-semibold">{contractData.clientAddress?.county}</span>, 
              identificat(ă)
            </p>
            <p className="mb-4">
              prin CI, seria <span className="font-semibold">{contractData.clientId?.series}</span>, 
              numărul <span className="font-semibold">{contractData.clientId?.number}</span>, 
              CNP <span className="font-semibold">{contractData.clientId?.cnp}</span>, 
              eliberat de <span className="font-semibold">{contractData.clientId?.issuedBy}</span> 
              la data <span className="font-semibold">{contractData.clientId?.issuedDate}</span>, 
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="ml-4 space-y-2">
                ...
                <p>b) adresa de E-mail a clientului: <strong>{contractData.clientEmail}</strong></p>
              </div>
            </div>
            ...
            <div>
              <p><strong>Art. 9.5</strong> Prezentul contract s-a încheiat în 2 exemplare azi, la data de {contractData.contractDate}, în mediul online prin corespondență la datele de contact menționate în Articolul 9.1, astfel:</p>
              <div className="ml-4 space-y-2">
