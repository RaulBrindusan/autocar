...
export default function AddExpenseDialog({ carId, onClose }: AddExpenseDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: addError } = await addExpense(expenseData);

      if (addError) {
        setError(`Eroare la adăugarea cheltuielii: ${addError}`);
        setLoading(false);
        return;
      }
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-xl w-full max-w-md shadow-2xl">
        <div className="flex justify-between items-center p-6 border-b">
          <h2 className="text-xl font-bold text-gray-900">Adaugă Cheltuială</h2>
          <button
            onClick={onClose}
            disabled={loading}
